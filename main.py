...
class Chat_With:
    ...
    def __init__(self, model: str, max_chat_log: int, ) -> None:
        ...
        self.tokenizer = tokenizer = AutoTokenizer.from_pretrained(model, use_fast=True, token=access_token)
    ...
    def forward(self) -> None:
        while True:
            ...
            self.prepare_prompt(user_input)
            model_inputs = tokenizer(user_input, return_tensors="pt").to("cuda:0")
            output = model.generate(**model_inputs, max_length=2048)
            response = f"Timothy: {tokenizer.decode(output[0], skip_special_tokens=True)}"
